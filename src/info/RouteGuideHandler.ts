    ...
    static add(res: directionObject) {
        for (let i = 0; i < res.direction.length; i++) {
            ...
            if (res.direction.length-1 != i) {
                routeGuide.insertAdjacentHTML('beforeend', HTMLBuilder.direcitonMainCard(e.min, e.line.name, `${e.from.name} >>> ${e.to.name}`, e.line.color, e.distance));
                routeGuide.insertAdjacentHTML('beforeend', HTMLBuilder.direcitonSubCard(e.to.name, e.line.name, res.direction[i+1].line.name));
            } else {
                routeGuide.insertAdjacentHTML('beforeend', HTMLBuilder.direcitonMainCard(e.min, e.line.name, `${e.from.name} >>> ${e.to.name}`, e.line.color, e.distance));
            }
        }
